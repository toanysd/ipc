...
function findBinary() {
  ...
  if (process.resourcesPath) {
    ...
    if (fs.existsSync(prodPathExe)) {
      console.log(`[sm-go2rtc] Found binary in resourcesPath: ${prodPathExe}`);
      return prodPathExe;
    }
    const prodPathPlatform = path.join(process.resourcesPath, binaryName);
    if (fs.existsSync(prodPathPlatform)) {
      console.log(`[sm-go2rtc] Found binary in resourcesPath: ${prodPathPlatform}`);
  ...
  if (fs.existsSync(devPathExe)) {
    console.log(`[sm-go2rtc] Found binary in dev resources: ${devPathExe}`);
    return devPathExe;
  }
  const devPathPlatform = path.join(__dirname, '..', 'resources', binaryName);
  if (fs.existsSync(devPathPlatform)) {
    console.log(`[sm-go2rtc] Found binary in dev resources: ${devPathPlatform}`);
  ...
  for (const dir of pathDirs) {
    ...
    if (fs.existsSync(fullPathExe)) {
      console.log(`[sm-go2rtc] Found binary in PATH: ${fullPathExe}`);
      return fullPathExe;
    }
    const fullPathPlatform = path.join(dir, binaryName);
    if (fs.existsSync(fullPathPlatform)) {
      console.log(`[sm-go2rtc] Found binary in PATH: ${fullPathPlatform}`);
      return fullPathPlatform;
    }
  }

  console.log(`[sm-go2rtc] Binary not found on disk, falling back to command name "${binaryName}"`);
  return binaryName;
}
...
`;
      ...
      console.log(`[sm-go2rtc] Created config file at ${configPath}`);
    } else {
      try {
    ...
    console.error(`[sm-go2rtc] Failed to ensure config file: ${err.message}`);
    throw err;
  }
}
...
function start(configDir) {
  if (isRunning()) {
    console.log('[sm-go2rtc] Process is already running');
  ...
  try {
    ...
    console.log(`[sm-go2rtc] Starting go2rtc from ${binaryPath} with config ${configPath}`);
    ...
    go2rtcProcess.stderr.on('data', (data) => {
      const lines = data.toString().split(/\r?\n/).filter((line) => line.trim().length > 0);
      for (const line of lines) {
        console.error(`[sm-go2rtc] ${line}`);
      }
    });

    go2rtcProcess.on('error', (err) => {
      console.error(`[sm-go2rtc] Process error: ${err.message}`);
    });

    go2rtcProcess.on('close', (code, signal) => {
      console.log(`[sm-go2rtc] Process closed (code: ${code}, signal: ${signal})`);
      go2rtcProcess = null;
      ...
      if (!isManuallyStopped) {
        if (retryCount < MAX_RETRIES) {
          retryCount++;
          console.log(`[sm-go2rtc] Unexpected exit. Restarting in ${RESTART_DELAY_MS / 1000}s... (Attempt ${retryCount}/${MAX_RETRIES})`);
          restartTimeout = setTimeout(() => {
            start(lastConfigDir);
          }, RESTART_DELAY_MS);
        } else {
          console.error(`[sm-go2rtc] Maximum restart retries (${MAX_RETRIES}) reached. Process will not be restarted.`);
        }
      }
    });
    ...
    console.error(`[sm-go2rtc] Error starting process: ${err.message}`);
    if (!isManuallyStopped && retryCount < MAX_RETRIES) {
      retryCount++;
      console.log(`[sm-go2rtc] Restarting in ${RESTART_DELAY_MS / 1000}s after start failure... (Attempt ${retryCount}/${MAX_RETRIES})`);
      restartTimeout = setTimeout(() => {
        start(lastConfigDir);
      }, RESTART_DELAY_MS);
    }
...
function stop() {
  ...
  if (go2rtcProcess) {
    console.log('[sm-go2rtc] Stopping go2rtc process...');
    try {
      go2rtcProcess.kill();
    } catch (err) {
      console.error(`[sm-go2rtc] Error stopping process: ${err.message}`);
    }
    go2rtcProcess = null;
  }
}
...
async function addStream(name, url) {
  try {
    ...
    console.log(`[sm-go2rtc] Added stream "${name}" -> "${url}"`);
    return result;
  } catch (err) {
    console.error(`[sm-go2rtc] Failed to add stream "${name}":`, err.message);
    throw err;
  }
}
...
async function removeStream(name) {
  try {
    ...
    console.log(`[sm-go2rtc] Removed stream "${name}"`);
    return result;
  } catch (err) {
    console.error(`[sm-go2rtc] Failed to remove stream "${name}":`, err.message);
    throw err;
  }
}
...
async function getStreams() {
  try {
    ...
    console.error('[sm-go2rtc] Failed to get streams:', err.message);
    throw err;
  }
}
...
async function getWebRTCOffer(name, offer) {
  try {
    ...
    console.error(`[sm-go2rtc] Failed to get WebRTC answer for stream "${name}":`, err.message);
    throw err;
  }
}
